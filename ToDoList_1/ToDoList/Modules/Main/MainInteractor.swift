...
class MainInteractor: MainInteractorProtocol {
    ...
    func saveNewTodo(_ newToDo: NewToDoEntity?) {
        ...
        let newRecord = ToDoRecord(id: newId, todo: newToDo?.todo, description: newToDo?.description, date: Date(), completed: false)
        toDoRecords?.append(newRecord)

        DispatchQueue.global(qos: .background).async {
            StorageService.shared.addTodo(newRecord)
